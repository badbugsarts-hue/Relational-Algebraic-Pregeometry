%=============================================================================
% UIDT Ontology v4.0 DRAFT-001 — Eliminating the Geometric Scalar:
% Vacuum Thermodynamics in Finite Algebraic Structures
% Author: Philipp Rietz (ORCID: 0009-0007-4307-1609)
% STATUS: PATCH_DRAFT under PI Decision D19 (DRAFT; PI signature pending).
...
% License: CC BY 4.0
%
% P1 SYNTHESIS NOTE (non-rendering):
% This file is the Phase-P1 skeleton of the v4.0 re-architecture. It retains
% the visual design language and evidence discipline of v3.9.9/-006 verbatim.
% It is an auditable research ontology DRAFT, NOT a merge authorization, NOT a
% canonical freeze, NOT a proof of UIDT, and NOT an evidence-class upgrade.
% AI-authored manuscript text is advisory only (AI_AUDIT_POLICY Sec. 1).
%
...
\fancypagestyle{uidtontology}{%
 ...
 \fancyhead[RE]{\small\textit{UIDT v4.0 DRAFT: Ontological Re-Architecture}}
 \fancyhead[LO]{\small\textit{P.\ Rietz}}
 \fancyfoot[C]{%
 \footnotesize\textcolor{darkgray}{%
 \copyright\ 2026 P.\ Rietz $\cdot$ CC BY 4.0 $\cdot$ DRAFT-001 (D19 pending) $\cdot$
...
%=============================================================================
\hypersetup{
 pdftitle={Eliminating the Geometric Scalar: Vacuum Thermodynamics in Finite Algebraic Structures (UIDT v4.0 DRAFT)}, pdfauthor={Philipp Rietz}, pdfsubject={Research-programme ontology of pregeometric finite algebraic structure and effective information fields}, pdfkeywords={Ontology, Noncommutative Geometry, Spectral Triple, Krajewski Diagram, Coarse-Graining, Effective Field Theory, UIDT}, colorlinks=true, linkcolor=black, citecolor=black, urlcolor=black, pdflang={en}
}
...
\vspace*{1.6cm}
\begin{center}
{\LARGE\bfseries Eliminating the Geometric Scalar:\\[0.25em]
Vacuum Thermodynamics in Finite\\[0.25em]
Algebraic Structures}\\[0.9em]
{\small\bfseries A Falsification-First Research Programme\catmark{D}\ for a Pregeometric Kernel,\\[0.25em]
the Thermodynamic Emergence of Continuity,\\[0.25em]
and the Demotion of \(S(x)\) to an Effective Information Field}\\[1.5em]
{\large\itshape Unified Information-Density Theory (UIDT) v4.0\\[0.25em]
Ontological Re-Architecture, DRAFT-001}\\[2.6em]
...
{\small 8~July~2026 --- DRAFT, not archived, not frozen}\\[2.2em]
...
\begin{governancebox}[Governance Status of This Draft]
This document is \textbf{DRAFT-001} of the UIDT v4.0 ontological
re-architecture. It authorizes nothing. It is not a canonical freeze, not a
merge into any protected repository path, and not an evidence-class upgrade
of any ledger claim. AI-authored manuscript text is advisory only
(AI\_AUDIT\_POLICY Sec.~1); all freeze, release, and archival decisions are
reserved to the Principal Investigator and, where required, to external peer
counter-signature. The canonical manuscript of record remains
\emph{UIDT Ontology v3.9.9} (\texttt{-006}) until the PI signs the
re-architecture decision recorded below.
\end{governancebox}
...
The three parts of this draft carry deliberately unequal epistemic weight.
Part~I (\emph{The Pregeometric Kernel}) is a \catmark{D}-class research
programme: it localizes the known obstructions (G1--G4), states lower
bounds that any admissible intermediate algebra must satisfy, and records a
Lean~4 formalization architecture whose axiom layer is, at the present
phase, a typed skeleton rather than a proof corpus. Part~II
(\emph{The Continuous Limit}) is capped at \catmark{E} wherever it touches
observer phenomenology, coarse-graining functors, or thermodynamic-limit
interpretation; no claim in Part~II validates $\Sx$ or derives the Born
rule. Part~III (\emph{The Effective Information Field}) carries the
framework's actual quantitative results at their unchanged ledger classes:
$\kappa = \KappaRG$\,\catmark{A}, $\lambda_S = \lambdaS$\,\catmark{A},
$v = \vVEV$\,\catmark{A}, $\gamma = \GammaGeom$\,\catmark{A-}
(calibrated, never derived), and $\Delta = \DeltaGap$\,\catmark{B}
(PI Decision D18; Limitation L12 barring any naive glueball identification
remains binding). Cosmological statements are capped at \catmark{C}
throughout. Nothing in Parts~I or~II upgrades anything in Part~III.
\end{evidencebox}
...
\setlength{\parskip}{0.65em plus 0.12em minus 0.04em}%
\setlength{\parindent}{0pt}%
\noindent This document re-architects the ontological foundations of the
Unified Information-Density Theory (\UIDT). Its predecessor postulated a
single continuous real scalar field, the vacuum information density $\Sx$,
as the ontological primitive. The present draft records the negative
results that motivate abandoning that posture: the $d^2=0$ obstruction to
any scalar-gradient gauge curvature\catmark{A}; the localization of the
$G_{\mathrm{SM}}$-origin gap to four jointly unsatisfied conditions
(G1--G4) on multiplicity, Wedderburn invariants, real structure, and
chirality; and an honest null result for dynamical selection of the
Standard-Model-like block pattern in matrix condensation. In consequence,
$\Sx$ is demoted --- under PI Decision D19, draft status --- from primitive
to macroscopic limit.

The draft is organized in three parts of deliberately unequal epistemic
weight. Part~I develops the candidate replacement primitive, a
\emph{pregeometric kernel} of finite real spectral-triple type with
Krajewski combinatorics, together with its Lean~4 formalization
architecture, strictly as a falsification-first research
programme\catmark{D}: block partitions, the real structure $J$, and
KO-dimension enter as inserted data whose endogenous selection is the open
problem, not as derived structure. Part~II examines the thermodynamic and
information-theoretic passage from discrete kernel to apparent continuity
--- coarse-graining maps, Markov-blanket-bounded observers, and the
compression limits of self-models --- capped at \catmark{E} and explicitly
barred from functioning as evidence for any primitive. Part~III retains the
framework's quantitative content as an effective information field: the
pure-$\mathrm{SU}(3)$ spectral gap $\Delta = \DeltaGap$\,\Bdelta\ as a
stress test under Limitation L12, the exact internal relations
$\kappa = \KappaRG$\,\catmark{A} and $\lambda_S = \lambdaS$\,\catmark{A},
the calibrated invariant $\gamma = \GammaGeom$\,\catmark{A-} (never
derived), and cosmological calibrations capped at \catmark{C}. Every
quantitative claim carries an explicit evidence class, every prediction a
falsification criterion, and every known limitation remains disclosed. The
framework has not solved quantum gravity, has not derived
$G_{\mathrm{SM}}$, and does not resolve the $H_0$ or $S_8$ tensions; this
draft exists to make the boundary of what it has and has not established
maximally explicit.
\endgroup
\end{abstract}
...
interpretation: the pregeometric-kernel hypothesis, the demotion of $\Sx$,
...
$\Sx \to G_{\mathrm{SM}}$ exists; the obstruction is concentrated in four
...
blinded metastability protocol for matrix condensation found that a
planted Standard-Model-like block configuration decays out of its class
within approximately ten Monte-Carlo steps at every tested coupling,
consistent with the published matrix-model literature. No verified
mechanism forcing a stable asymmetric block vacuum is known; that search
remains an open \catmark{D}/\catmark{E} programme.
...
The demotion of $\Sx$ under PI Decision D19 (draft;
\hyperref[box:pi-decision-d19]{governance page}) is an ontological reassignment, not a physics
...
level; routes through $A = \dd S(x)$ on a smooth manifold are excluded at
...
exactly once, and exactly as governance condition~(ii) of D19 requires: as
...
$\Sx$ as candidate primitive & \catmark{D} & conditions of the
...
G-01 & No gapless ab initio $\Sx \to G_{\mathrm{SM}}$ in the current
...
%=============================================================================
\newpage
\part{The Continuous Limit\texorpdfstring{\ \catmark{E}}{[E]}-Capped}
...
% D19 reframe: the microscopic level of the compression picture is now the
% pregeometric kernel candidate of Part I; S(x) belongs to the effective,
...
\section{The Thermodynamic Passage: From Kernel to Apparent Continuum}
...
dynamics, with the effective information field $\Sx$ of Part~III as the
...
establishes the kernel, rehabilitates $\Sx$ as primitive, or acquires
...
\cref{subsec:strata}, applied to a new domain. Three kinds of statement
...
\noindent Under D19 the referent of ``underlying structure'' shifts, and
the shift improves the picture's internal coherence. In v3.9 the
observer compressed a relational structure built on the primitive
$\Sx$; in the present draft the microscopic level is the pregeometric
kernel candidate of Part~I --- itself held at \catmark{D} --- and the
effective information field $\Sx$ belongs to the \emph{output} side of
...
category of microscopic descriptions --- under D19, descriptions at the
...
and it does not derive subjective experience from the kernel, from
$\Sx$, or from anything else in its corpus.\catmark{A/E} The self-model
account is a proposal about the \emph{structure} of conscious systems
...
\catmark{E\,pending} quantity, not an established cognitive
...
particle with its own phenomenology, carried strictly at \catmark{E}\
until explicitly constructed.\catmark{A/E} Constructing one, if it can
...
\catmark{E\,pending} (ledger/external bridge required), never an
...
as its content. Under the D19 re-architecture one adjustment is mandatory:
the zero-compression limit is no longer identified with a primitive scalar.
\Sx\ is an effective, coarse-grained order parameter (Part~III), not the
...
relational dynamics, of which \Sx\ is the coarse-grained effective
...
agency; \Sx\ effective only) through Class~III (extreme compression, inert
...
phenomenology is not evidence for $\Sx$ --- and now, a fortiori, not
...
\bottomrule
\endfoot
Primitive & Pregeometric kernel (finite spectral triple); \Sx\ effective,
...
P-01 & Phenomenology is not evidence for $\Sx$ or for the kernel &
...
%=============================================================================
% PART III — THE EFFECTIVE INFORMATION FIELD
%=============================================================================
\newpage
\part{The Effective Information Field}
\label{part:effective-field}

\noindent Part~III carries the framework's quantitative content in its
demoted but undamaged form: $\Sx$ as a macroscopic effective information
field coupled to an already-present Yang--Mills sector. Every ledger
result retains its v3.9 evidence class unchanged --- the exact internal
relations, the calibrated invariant under its permanent anti-derivation
discipline, the spectral gap under PI Decision D18 and Limitation L12, the
benchmark batteries, and the cosmological calibrations at their
\catmark{C} cap. The demotion of $\Sx$ removes an ontological claim; it
does not add, subtract, or reclassify a single quantitative result.

%%ANCHOR-P4-PART-III-CONTENT%%

%============================ P4 PART III CONTENT ==========================
% Inserted at ANCHOR-P4-PART-III-CONTENT by fail-fast patch script.
% Relocates UIDT Ontology v3.9.9 (-006) Parts I-IV substance under D19:
...
\section{Axiomatics of the Effective Information Field}
\label{sec:axiomatics}

\noindent A part that reasons about the spectral gap $\Delta$, the
...
quantities actually live. This section states the four axioms of the
effective sector, the explicit effective Lagrangian, and the dimensional
bookkeeping that fixes the theory's status as an effective field theory.
...
\begin{axiom}[Effective-Field Postulate (D19 form)]
\label{ax:primacy}
There exists a real scalar effective field $\Sx$, the vacuum information
density, of canonical mass dimension $[\Sx]=1$. Under PI Decision D19
(draft), $\Sx$ is \emph{not} an ontological primitive: it is postulated
...
this axiom --- $\Sx$ as sole fundamental degree of freedom, with all
structure emergent from its dynamics (Variant~B) --- is recorded in
\cref{subsec:variant-b} as the superseded grammar.\catmark{A/E}
\end{axiom}

\begin{axiom}[Gauge-Invariant Self-Interaction]
\label{ax:gauge}
The dynamics of $\Sx$ are governed by a Lagrangian density
...
configuration of $\Sx$. A candidate parametrisation is
\begin{equation}
...
\textbf{Axioms.} Effective-Field Postulate \catmark{A/E} (D19 form);
Gauge-Invariant Self-Interaction \catmark{A} (EFT, dimension-5
...
\section{Unmarked State, First Distinction, and the Superseded Variant B}
...
unique primitive of nature --- and stamped the choice \catmark{E}\ as an
interpretive commitment.\catmark{A/E}
...
Neither is a constituent of the genesis.\catmark{A/E} Under D19 the
...
QBism --- remain \emph{lenses}, not definitions of the
primitive.\catmark{A/E}
...
\begin{evidencebox}[The Effective-Field Boundary]{catA}
...
decomposition is a museum artifact: the integer $16339$ is prime, which
proves that the split was \emph{constructed} to land on the target
rather than \emph{found} by any mechanism, and the construction has no
physical origin. Perfect internal closure, in this case, certifies only
that someone did the arithmetic correctly.\catmark{A}
...
It is \emph{not} \catmark{A}, and it is not derived from
renormalisation-group first principles.\catmark{A-} To be precise about
what ``calibrated'' means: $\gamma$ is fixed by matching to the kinetic
vacuum-expectation structure of $\Sx$ in the effective Lagrangian of
\cref{eq:lagrangian} --- a calibration to an internal kinetic reference,
not a derivation from a renormalisation-group flow. The public-facing
...
$\Lambda_{\overline{\mathrm{MS}}}$-normalised forms.\catmark{B pending}
...
or scale bridge; only after those are exhausted does a persistent
mismatch rise to a serious tension against the framework as a
whole.\catmark{A/D}
...
specified which loophole its construction exploits. Under D19 this
...
framework treats it as a decisive result --- and because, under D19, it
...
versus enriched primitive --- has been taken: D19 commits the research
...
Category~\catmark{C}: calibrated to external data, never derived from
$\Sx$, and never presented as resolving an observational tension. The
...
constraints rather than derived from $\Sx$.\catmark{C}
...
\section{The Closed-Circle Sequence and Final Claims}
\label{sec:closed-circle}

\subsection{The Empirical-to-Ontology Return Path}
\label{subsec:return-path}

The architecture of this draft traces a single large loop, and it closes
without inflation. One begins from the empirical and lattice stress
...
through an explicit uncertainty and covariance model; identifies
structural successes and failures (\catmark{A/D}); and interprets last,
at \catmark{E}. The closure is governed throughout by the two structural
limits this draft has doubled down on: the $d^2=0$ obstruction
(\cref{prop:d2-obstruction}) and the localized
$G_{\mathrm{SM}}$-origin gap (\cref{sec:g1g4}). The circle does not
close on a triumph; it closes on a disciplined accounting, and closing
it without evidence inflation is the whole achievement of the
synthesis.\catmark{A}
...
unchanged from v3.9 except where D19 renames the grammar (ONT-11).}
\label{tab:claims}\\
\toprule
...
ONT-11 & Under PI Decision D19 (draft), $\Sx$ is demoted from primitive to effective information field; the kernel candidate of Part~I holds the primitive office at \catmark{D}. & \catmark{D} & III \\
\end{longtable}
}
...
$G_{\mathrm{SM}}$ --- from the scalar $S(x)$, and equally none from the
...
\textbf{(4)} No validation of $S(x)$ or of the kernel by psychedelic,
...
\textbf{F3: Complete benchmark accounting.} Every external benchmark
carries a verified source, an uncertainty budget, and a covariance
model. \catmark{A/B\,pending}\\[0.3em]
...
T2 & $\Delta = \DeltaGap$ evidence class. & Would sit \catmark{E\,pending} absent external work; determination D18 sets \catmark{B}\ (\cref{box:pi-override-delta}). & External peer counter-signature. \\
...
T5 & $E_T$ observer/neural scale bridge. & Absent; $\sim$6--9 orders of magnitude. \catmark{A/E} & Derive or refute a principled scale bridge. \\
...
% adapted to the D19 architecture. Dropped in error during P2-P4; recovered in
...
\begin{governancebox}[What D19 Does to the Structural-Realist Position]
Under PI Decision D19 the identity of the candidate constituting
...
\UIDT\ under D19 has moved \emph{toward} that austerity, replacing a
...
necessity or anthropic selection. Under D19 even that answer contracts:
...
result; under D19 its candidate structure is a \catmark{D} research
...
that quietly declares victory. This draft runs the test twice: once
against the v3.9 claim it inherits, and once against the D19
re-architecture it introduces.
...
the internal kinetic vacuum structure of $\Sx$
(\cref{subsec:gamma-discipline}), independently of cosmology, so a
...
\subsection{Second Round: The D19 Re-Architecture}
\label{subsec:adversary-d19}

\paragraph{Adversarial argument.}
The re-architecture, says the adversary, is an evasion dressed as
...
\textbf{Round 2 (D19).} All three charges conceded: the kernel is an
...
The only proof-critical arithmetic asserted in this draft is the exact
internal coupling constraint of \cref{eq:lagrangian}. Working precision
...
The Lean~4 statements quoted in Part~I are transliterations of the
repository sources at commit \texttt{61bc24da}. The audit reported in
...
is asserted anywhere in this draft. Reproduction requires fetching the
stated commit and repeating the scan; Findings F1--F3 are stated so that

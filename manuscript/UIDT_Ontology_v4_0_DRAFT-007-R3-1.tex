...
must be held rigorously apart.\catmark{A}\,\catmark{E}
...
historical scalar ansatz, or from anything else in its corpus.\catmark{A}\,\catmark{E} The self-model
...
until explicitly constructed.\catmark{A}\,\catmark{E} Constructing one, if it can
...
model. \catmark{A}\,\catmark{B}\ (\emph{pending external verification})\\[0.3em]
...
T4 & SVZ leading-order topology route versus lattice topology. & \catmark{A}\,\catmark{B}\ (\emph{pending external verification}); SVZ route is museum-only. & Audited lattice $\chi_{\mathrm{top}}$ ratios. \\
T5 & $E_T$ observer/neural scale bridge. & Absent; $\sim$6--9 orders of magnitude. \catmark{A}\,\catmark{E} & Derive or refute a principled scale bridge. \\
...
\cref{subsec:variant-b} as the superseded grammar.\catmark{A}\,\catmark{E}
\end{axiom}
...
\textbf{Axioms.} Effective-Field Postulate \catmark{A}\,\catmark{E} (D19 form);
...
interpretive commitment.\catmark{A}\,\catmark{E}
...
Neither is a constituent of the genesis.\catmark{A}\,\catmark{E} Under D19 the

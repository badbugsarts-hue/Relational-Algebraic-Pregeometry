...
%  R5 manuscript post-application checks .. baseline bytes verified; live Lean,
%                                             remote CI, and PROV-002 remain open
%
% --- SOURCE STATUS -----------------------------------------------------------
...
\begin{governancebox}[Live Lean Status Is Not Established by This Manuscript]
The historical findings F1--F3 below remain part of the audit record, but they
must not be projected onto a later branch without raw-file reconciliation.
The current commit tree, complete import graph, build log, axiom inventory,
and whole-repository \texttt{sorry}/\texttt{admit} scan remain external
requirements under GAP-22. Until those artefacts are supplied, this manuscript
makes no claim that the historical defects persist and no claim that they
have been repaired.
\end{governancebox}
...
decision-procedure evaluation. At the audited historical head the register statement was false: the
...
In the audited historical snapshot, a wrapper in the candidate-instance layer defines a carrier to be
...
Of the 51 open obligations in the audited historical head, the register covers only the five
...
either way \\
L-01 & Historical scan reported no \texttt{sorry} in Foundation/RealStructure/FOC/Krajewski/DeltaScale at \texttt{61bc24da}; no live status is asserted & \catmark{A} (snapshot audit fact) & raw live-tree reconciliation changing the record \\
...
GAP-22 & Live Lean reconciliation & Current commit tree, build log, axiom/sorry
inventory, and line-by-line manuscript reconciliation. & \catmark{A}\\
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.25em]
\item Lean findings F1--F3 are historical findings at
\texttt{61bc24da}. Their current status is unresolved until the live raw tree,
build log, axiom inventory, and whole-repository proof-debt scan are
reconciled. Any repair upgrades only the formal software layer; physical
